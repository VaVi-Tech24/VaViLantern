(function(root){
 ...
 function step(p,dt,seed,time,onEvent=()=>{}){if(!p.alive||p.completed)return;const cfg=travelers[p.character],stage=level(p.level);const old=p.x;const inAir=p.y>0||p.vy!==0;
  ...
  for(const o of obstacles(seed,old-260,p.x+1,p.level)){const inside=p.x>=o.x&&p.x<o.x+o.width;if(inside){if(o.type==='gap'&&p.y<.15){hit(o.wide?'Use '+o.jumpsRequired+' timed jumps to cross this break.':'Jump over the missing planks.');break;}if(o.type==='beam'&&p.slide<=0){hit('Slide beneath the low branch.');break;}if(o.type==='rope'&&p.y<3){hit('Hop over the anchored rope.');break;}if(o.type==='crow'&&!p.passed['c'+o.id]&&p.y<3&&p.slide===0){p.flame=Math.max(0,p.flame-12);p.passed['c'+o.id]=true;onEvent('crow',p);}}
