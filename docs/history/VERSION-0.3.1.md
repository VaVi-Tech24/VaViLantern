...
Beam and rope obstacles sit across the deck center with diagonal bases and crosspieces matching the plank perspective. Birds and fog hazards align to the same running surface. Collision rules remain unchanged.
...
Hazard placement now accounts for each gap's width and landing/recovery room, preventing overlap in the shorter opening level. Opening collapse pressure is gentler and rises toward the previous final-level pressure. Walking alone still loses to the collapsing bridge.

Campaign, refill, fall/splash, UI smoke and service checks passed. Browser screenshots verified the starting gateway, typography and diagonal obstacle placement. Android output is a debug-signed testing APK and unsigned release bundle.
