...
`assets/` retains original/reference material, including the cosmic lantern image and Jonah concept. Android build 27 uses the selected Jonah-and-torch bridge artwork in app/src/main/res/drawable-nodpi/lantern_app_icon.png. It preserves the supplied image, including the VaViTech24 badge. The adaptive launcher adds 27% padding to protect the complete square image within round masks. Android also serves this image for its in-game header logo; website branding is unchanged.
