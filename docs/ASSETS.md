...
The header uses the user-supplied VaVi Tech 24 PNG (`web/public/vavi-tech-logo.png`). It is preserved as supplied, with responsive layout sizing. The game title is VaVi Lantern; the footer reads copyright VaVi Tech 2026.
...
`assets/` retains original/reference material, including the cosmic lantern image and Jonah concept. Android's launcher drawable retains the earlier cosmic lantern icon; the requested VaVi Tech logo change applies to the game header, not an unrequested launcher replacement.
