...
VaVi Lantern stores unlocked and completed campaign levels, character choices, local scores and settings on your device. This version has no advertising SDK, analytics SDK, in-app purchases or location access. Android backup is disabled for the app. Clearing app data or uninstalling removes local progress. Version 0.2 no longer records ghosts; any older ghost data from version 0.1 stays unused until app data is cleared.

Online play is optional. When you connect to the configured game service, it receives your chosen display name, game inputs and connection metadata such as the IP address needed to deliver the service. The server calculates race state and results. Display names, distances and scores may appear publicly in the rolling seven-day leaderboard. Do not use your real name or personal contact information as your display name.
